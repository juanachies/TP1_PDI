...
def verificar(img):
    ...
    axes[1].set_title("Imagen con umbralado manual")

    # Definimos nuestra imagen booleana
    img_bool = img_th == 0 
    axes[2].imshow(img_bool, cmap='gray')
    axes[2].set_title("Imagen booleana")

    plt.tight_layout()
    plt.show()

    # Detectamos líneas horizontales (renglones)
    ...
    # Procesamos cada renglón y detectamos líneas horizontales (celdas) 
    celdas = []
    for idx, (y1, y2, sub) in enumerate(subimgs):
        sub_bool = sub < th

        # Detectamos columnas en este renglón 
    ...
    # Definimos variables para almacenar la cantidad de letras de cada celda
    cant_letras = [[0]*len(cr) for cr in celdas]
    cant_palabras = [[0]*len(cr) for cr in celdas]

    # Subplots para las celdas
    n_rows = len(celdas)
    ...
    # Normalizamos el shape de axes para indexarlo como axes[row, col]
    ...
    for idx, celdas_renglon in enumerate(celdas):
        ...
        for j, celda in enumerate(celdas_renglon):
            ...
            # Recorremos los componentes detectados, excepto el fondo 
            for i in range(1, len(stats)):
                ...
                # Filtramos componentes muy pequeños (ruido)
                if area < 3:
                    ix_valido[i] = False
            ...
            # Almacenamos las letras para mostrarlas
            letras = []
            ...
            # Mostramos cada renglon con letras detectadas
            ax.imshow(celda_color[..., ::-1])
            ax.set_title(f"Renglón{idx+1}, Celda{j+1}", fontsize=8)
            ax.axis('off')
            
            # Encontramos el tipo de formulario
            if idx == 0 and j == 0:
    ...
    # Subplot de las filas y renglones con sus letras detectadas 
...
# Resumen del estado de los formularios
